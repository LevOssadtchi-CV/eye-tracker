...
CALIBRATION_DONE = ""
EMA_ALPHA = 0.3  # Коэффициент сглаживания (0 < alpha < 1, меньшее значение — сильнее сглаживание)
    ...
    while True:
        ...
        if results.multi_face_landmarks:
            ...
            for face_landmarks in results.multi_face_landmarks:
                ...
                crop, _, _, _, _ = crop_eyes(frame, rect)
                ...
                if ema_x_rel is None:
                    ema_x_rel = pred_x_rel
                    ema_y_rel = pred_y_rel
                else:
                    ema_x_rel = EMA_ALPHA * pred_x_rel + (1 - EMA_ALPHA) * ema_x_rel
                    ema_y_rel = EMA_ALPHA * pred_y_rel + (1 - EMA_ALPHA) * ema_y_rel

                # Преобразование в экранные координаты
                point_x = int(np.clip(ema_x_rel, 0, 1) * screen_w)
                point_y = int(np.clip(ema_y_rel, 0, 1) * screen_h)

                # Рисуем точку на канвасе
                cv2.circle(canvas, (point_x, point_y), POINT_RADIUS, POINT_COLOR, -1)

                # Рисуем центр экрана для калибровки (красный крест)
                cv2.line(canvas, (screen_w//2 - 10, screen_h//2), (screen_w//2 + 10, screen_h//2), (0, 0, 255), 2)
                cv2.line(canvas, (screen_w//2, screen_h//2 - 10), (screen_w//2, screen_h//2 + 10), (0, 0, 255), 2)

        # Отображение инструкции
        if not calibrated:
            cv2.putText(canvas, CALIBRATION_INSTRUCTION, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        else:
            cv2.putText(canvas, CALIBRATION_DONE, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        ...
        if key == 27:  # Esc для выхода
            ...
            calibrated = True
            # Сбрасываем EMA после калибровки
            ema_x_rel = None
            ema_y_rel = None
